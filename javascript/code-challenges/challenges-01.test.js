...
const createList = (availableItems) => {
  ...
  availableItems.forEach(item => {
    if (item.available) {items.push(item.name);}
  });
  return items;
};
...
const fizzbuzz = (arr) => {
  ...
  arr.forEach(number => {
    ...
    let result = number;
    if (isDivisibleBy3 && isDivisibleBy5) result = 'Fizz Buzz';
    if (isDivisibleBy3 && !isDivisibleBy5) result = 'Fizz';
    if (!isDivisibleBy3 && isDivisibleBy5) result = 'Buzz';
