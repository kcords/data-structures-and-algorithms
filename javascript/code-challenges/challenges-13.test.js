...
const longestString = (arr) => {
  const [index] = arr.reduce(([longestIdx, len], string, idx) => {
    if (string.length > len) return [idx, string.length];
    return [longestIdx, len];
  }, [-1, 0]);
  return index;
};
...
const standardizePhoneNumbers = (arr) =>
  arr.map((str) => str.replace('-', '').replace('(', '').replace(') ', ''));
...
const unenrollBrook = (arr) => arr.map(roster => roster.filter(name => !name.includes('Brook')));
...
const characterByIndex = (arr) => arr.map((string, idx) => string[idx]);
