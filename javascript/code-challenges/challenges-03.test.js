...
const typeNum = (arr) => arr.filter(value => Number.isInteger(value));
...
const containsAnd = (arr) => arr.filter(item => item.toLowerCase().includes('and'));
...
const oddValues = (arr) => arr.filter(integer => integer % 2 > 0);
...
const getBaseStatGreaterThan = (arr, minBaseStat)=> arr.filter(({baseStat}) => baseStat > minBaseStat);
...
const getCharactersWithoutChildren = (arr) => arr.filter(character => !('children' in character));
