...
const totalSum = (input) =>
  input.reduce((sum, arr) =>
    sum + arr.reduce((currSum, val) =>
      currSum + val, 0), 0);
...
const divisibleByFiveTwoToThePower = (input) => input.map(
  arr => arr.filter(
    val => Number.isInteger(val) && val % 5 === 0)
...
let findMaleAndFemale = (data) => data.filter(({gender}) => gender === 'male' || gender === 'female').map(({name}) => name).join(' and ');
...
let findShortest = (data) =>
  data.reduceRight((shortest, current) =>
    parseInt(current.height) < parseInt(shortest.height) ? current : shortest).name;
