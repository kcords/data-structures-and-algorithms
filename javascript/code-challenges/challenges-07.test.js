...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(ingredient => {
    ...
    let nextSpace = 0;
    while (count < 2) {
      nextSpace = ingredient.indexOf(' ');
      ingredient = ingredient.slice(nextSpace + 1);
      count += 1;
    }
...
const splitFoods = (recipe) => recipe.ingredients.map(
...
const totalSumCSV = (str) => str.split(',').reduce((sum, num) => sum + parseInt(num), 0) ;
...
const extractVowels = (str) => {
  ...
  return str.split('').reduce((arr, char) => {
    ...
    isVowel ? arr[1] += char : arr[0] += char;
    arr[1] = arr[1].split('').sort((a, b) => a > b ? 1 : -1).join('');
    return arr;
  }, ['', '']);
};
